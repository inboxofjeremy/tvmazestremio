...
if (/[\u4E00-\u9FFF\u3040-\u30FF]/.test(name)) return true;
...
// ==========================
// TVMAZE EPISODES-BY-DATE FALLBACK (NEW)
// ==========================
async function fetchTvmazeEpisodesByDate(showId, todayStr) {
const eps = [];
...
if (Array.isArray(list)) eps.push(...list);
```
...
// --- 2) TMDB fallback mapped to TVMaze
...
// --- 3) NEW: TVMaze episodes-by-date fallback (per show)
for (const [id, entry] of showMap.entries()) {
const showId = entry.show.id;
const extra = await fetchTvmazeEpisodesByDate(showId, todayStr);

```
if (extra.length) entry.episodes.push(...extra);
```
...
description:
"English shows aired in the last 10 days. Includes TMDB and per-show TVMaze fallbacks.",
catalogs: [
{
type: "series",
